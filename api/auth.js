// Vercel Serverless Function for PIN Authentication
// Database connection to remote MySQL
...
async function initSettingsTable(conn) {
...
async function verifyPin(conn, pin) {
    if (!pin) return false;
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'POST') {
            ...
            const isValid = await verifyPin(conn, pin);
            if (isValid) {
        ...
        if (req.method === 'PUT') {
            ...
            const isValid = await verifyPin(conn, currentPin);
            if (!isValid) {
                return res.status(401).json({ success: false, error: 'PIN saat ini salah' });
            }

            await conn.execute("UPDATE settings SET value = ? WHERE key_name = 'user_pin'", [newPin]);
